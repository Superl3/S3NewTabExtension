...
test("core layout modules use shared array fallback helper", async () => {
  const moduleUrls = [
    ...
    new URL("../core/dock-widgets-render.js", import.meta.url),
  ...
  const dockWidgetsRenderSource = await fs.readFile(new URL("../core/dock-widgets-render.js", import.meta.url), "utf8");
  assert.doesNotMatch(dockWidgetsRenderSource, /dockedInstances\?\.\(\) \|\| \[\]/);
...
test("Flex worktime widgets share row helpers instead of local copies", async () => {
  ...
  const localHelperPattern =
    /^function (applyTemplate|formatDurationMinutes|formatSourceError|formatSyncedLabel|formatTimeFromRef|normalizeCachedRow|normalizeFlexHomeScrapeRow|normalizeTabId|normalizeWorktimeRow|resolveDetailUrl|resolvePathValue|resolveSyncState|sanitizePlaceholderMap|sourceModeLabel|toCachedRow)\(/m;
...
test("Flex worktime cache uses core number helpers", async () => {
  ...
  assert.match(source, /core\/utils\/number\.js/);
  assert.match(source, /toPositiveInteger\(options\.maxEntries, 1\)/);
  assert.doesNotMatch(source, /^function normalizeMaxEntries\(/m, "flexWorktimeCache.js");
...
test("GitHub widgets share repository and API helpers", async () => {
  ...
  for (const moduleUrl of moduleUrls) {
    ...
    assert.doesNotMatch(source, /\.map\((?:normalizeCachedItem|normalizeCachedPullItem|toCachedPullItem)\)\s*\n\s*\.filter\(Boolean\)/, moduleUrl.pathname);
...
test("GitHub widgets use shared array fallback helper", async () => {
  ...
  assert.match(prListSource, /sortPullItemsByUpdatedAt\(cachedPullItems\)/);
  assert.match(prListSource, /sortPullItemsByUpdatedAt\(mapped\)/);
  assert.equal(
    Array.from(prListSource.matchAll(/\.sort\(\(a, b\) => b\.updatedAt - a\.updatedAt\)/g)).length,
    1
  );
  ...
  assert.doesNotMatch(inboxSource, /Array\.isArray\(rawConfig\?\.cacheReviewItems\)/);
  assert.doesNotMatch(inboxSource, /Array\.isArray\(pull\?\.requested_(reviewers|teams)\)/);
...
test("Monday widgets share active auth connection predicate", async () => {
  const moduleUrls = [
    new URL("../widgets/mondayAssigned.js", import.meta.url),
    new URL("../widgets/mondayMeetingNote.js", import.meta.url)
  ];
  const localActiveConnectionPattern =
    /const configuredToken = normalizeText\(config\?\.accessToken\)[\s\S]*?config\.connectorUrl === sessionConnectorUrl/;

  for (const moduleUrl of moduleUrls) {
    const source = await fs.readFile(moduleUrl, "utf8");
    assert.match(source, /hasActiveAuthConnection/, moduleUrl.pathname);
    assert.doesNotMatch(source, localActiveConnectionPattern, moduleUrl.pathname);
  }
});
...
test("auth widgets share connector result session creation", async () => {
  const moduleUrls = [
    new URL("../widgets/aiChat.js", import.meta.url),
    new URL("../widgets/mondayAssigned.js", import.meta.url),
    new URL("../widgets/mondayMeetingNote.js", import.meta.url)
  ];
  const localStoredSessionPattern =
    /authSessionStorage\.save\(\{\s*connectorUrl[\s\S]*?accountLabel\s*\}\)/;
  const localAssignedSessionPattern =
    /storedSession\s*=\s*\{\s*connectorUrl[\s\S]*?accountLabel:\s*result\.accountLabel/;

  for (const moduleUrl of moduleUrls) {
    const source = await fs.readFile(moduleUrl, "utf8");
    assert.match(source, /createStoredAuthSessionForConnectorResult/, moduleUrl.pathname);
    assert.doesNotMatch(source, localStoredSessionPattern, moduleUrl.pathname);
    assert.doesNotMatch(source, localAssignedSessionPattern, moduleUrl.pathname);
  }
});
...
test("Monday widgets share list array fallback normalization", async () => {
  ...
  assert.doesNotMatch(assignedSource, /rawItems \|\| \[\]/);
  assert.doesNotMatch(assignedSource, /for \(const (?:issue|group) of (?:items|boardGroups) \|\| \[\]\)/);
  assert.doesNotMatch(assignedSource, /groupIssuesByGroup\([^,\n]+ \|\| \[\], [^)]+ \|\| \[\]\)/);
  assert.match(assignedSource, /function isDoneLabel\(/);
  assert.doesNotMatch(assignedSource, /DONE_(?:GROUP_TITLES|STATUS_LABELS)/);
  assert.doesNotMatch(assignedSource, /^function isDone(?:GroupTitle|StatusLabel)\(/m);
  assert.doesNotMatch(assignedSource, /^function normalizeColumnIds\(/m);
  assert.doesNotMatch(assignedSource, /\.map\(\(value\) => normalizeColumnSelector\(value\)\)/);
  ...
  assert.doesNotMatch(meetingNoteSource, /if \(!normalized\) \{\s*return "";\s*\}/);
});
